...
function splitTextByHeight(text, maxHeight) {
    if (!ui.textBox || !text) return [text];
    ...
    if (boxWidth < 100 && ui.dialogueBox) {
        ...
        if (!isNaN(cssWidth) && cssWidth > 100) boxWidth = cssWidth - 20;
        else return [text]; // 放棄分頁
    }
    ...
    testBox.style.width = boxWidth + "px";
    ...
    testBox.style.wordBreak = "break-word";
...
function nextStep() {
    ...
    if (state.textQueue && state.textQueue.length > 0) {
        ...
        stepData = { ...lastSnapshot.stepData, text: nextChunk };
    } else {
        if (state.index >= scenario.length) return;

        const rawStep = { ...scenario[state.index] };
        ...
        stepData = { ...rawStep, speaker: displaySpeaker };
        state.history.push({ index: state.index, speaker: stepData.speaker || "", text: stepData.text || "" });
        state.index++;
        state.textQueue = [];

        if (stepData.text && ui.dialogueBox) {
            ...
            const maxHeight = totalHeight - pt - pb - 20;

            if (maxHeight > 50) {
                const pages = splitTextByHeight(stepData.text, maxHeight);
                stepData.text = pages.shift();
...
function initGame() {
    if(!ui.gameScreen) return;
    console.log("Engine Started: Fixed paging + typewriter");
